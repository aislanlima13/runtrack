package com.aislan.run.presentation.active_run.service
...
import com.aislan.core.presentation.ui.formatted
...
class ActiveRunService: Service() {
    ...
    /** notification channel são os tipos diferentes de notificações que podem ser exibidas **/
    private val baseNotification by lazy {
        NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setSmallIcon(com.aislan.core.presentation.designsystem.R.drawable.logo)
    ...
    private fun start(activityClass: Class<*>) {
        if (!isServiceActive) {
            ...
            /* contentIntent = settar qual activity será aberta quando o usuário clicar na notificação */
            val activityIntent = Intent(applicationContext, activityClass).apply {
                data = "runtrack://active_run".toUri()
                addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP)
            }

            /* pendingIntent =  "Uma PendingIntent em si é simplesmente uma referência a um token mantido
            pelo sistema, que descreve os dados originais usados para recuperá-la*/
